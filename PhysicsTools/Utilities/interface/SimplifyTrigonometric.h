...
namespace funct {
  ...
  TEMPL(T1) struct Ratio<SIN_S(A), COS_S(A)> : 
  ...
  TEMPL(T1) struct Ratio<SIN_S(A), TAN_S(A)> : 
  ...
  TEMPL(T1) struct Product<COS_S(A), TAN_S(A)> :
    public SimplifySTProduct<A> { };
  ...
  template<TYPT2,
    bool parametric = Parametric<A>::value || Parametric<B>::value>
  ...
  TEMPL(T2) struct SimplifySTnProduct<A, B, false> {
    ...
    COMBINE(POWER_S(COS_S(A), B), POWER_S(TAN_S(A), B),
	    pow(sin(_1._1._), _1._2 ));
  };
  
  TEMPL(T2) struct Product<POWER_S(COS_S(A), B), 
    POWER_S(TAN_S(A), B)> :
    public SimplifySTnProduct<A, B> { };
  
  TEMPL(N1T1) struct Product<POWER_S(COS_S(A), NUM(n)), 
  ...
  struct SimpifyS2C2Sum {
    typedef SUM(PROD(NUM(n), SIN2(A)), 
		PROD(NUM(m), COS2(A))) type;
    COMBINE(PROD(NUM(n), SIN2(A)), 
	    PROD(NUM(m), COS2(A)), _1 + _2);
  };
  ...
  TEMPL(N2T1) struct SimpifyS2C2Sum<n, m, A, false> {
    ...
    typedef SUM(SUM(PROD(NUM(n - p), SIN2(A)), 
		    PROD(NUM(m - p), COS2(A))), NUM(p)) type;
    COMBINE(PROD(NUM(n), SIN2(A)), 
	    PROD(NUM(m), COS2(A)), (num<n - p>() * _1._2 + 
				    num<m - p>() * _2._2) + num<p>());
  };
  
  TEMPL(T1) struct Sum<POWER_S(SIN_S(A), NUM(2)), 
    POWER_S(COS_S(A), NUM(2))> :
    public SimpifyS2C2Sum<1, 1, A> { };
  
  TEMPL(T1) struct Sum<POWER_S(COS_S(A), NUM(2)), 
  ...
  TEMPL(N2T1) struct Sum<PROD_S(NUM(n), POWER_S(SIN_S(A), NUM(2))), 
    PROD_S(NUM(m), POWER_S(COS_S(A), NUM(2)))> :
    public SimpifyS2C2Sum<n, m, A> { };
  
  TEMPL(N2T1) struct Sum<PROD_S(NUM(m), POWER_S(COS_S(A), NUM(2))), 
